...
int main() {
	...
	GLFWwindow *window =
		glfwCreateWindow(WIDTH, HEIGHT, "HW4", nullptr, nullptr);
	...
	GLuint textureID =
		glGetUniformLocation(textureShader.Program, "myTextureSampler");
	...
	// Declare and initialize UV buffer object ID
	GLuint uvBuffer;
	glGenBuffers(1, &uvBuffer);

	// Bind UV buffer to GL_ARRAY_BUFFER target
	glBindBuffer(GL_ARRAY_BUFFER, uvBuffer);

	// Copy UV coordinates data to the currently bound buffer object
	glBufferData(GL_ARRAY_BUFFER, sizeof(uv), uv, GL_STATIC_DRAW);

	// Set up vertex attribute pointers for UV coordinates in vertex shader
	GLuint uvAttrib = glGetAttribLocation(textureShader.Program, "vertexUV");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		GLint projLoc =
			glGetUniformLocation(textureShader.Program, "projection");
